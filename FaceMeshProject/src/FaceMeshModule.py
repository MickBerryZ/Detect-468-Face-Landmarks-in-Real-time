...
def main():
    cap = cv2.VideoCapture("FaceMeshProject/Videos/travel5.mp4")
    previousTime = 0
    detector = FaceMeshDetector(maxFaces=2)
    while True:
        success, img = cap.read()
        # img = detector.findFaceMesh(img, False) # don't draw face
        img, faces = detector.findFaceMesh(img)  # draw face
        if len(faces)!= 0:
            print(faces[0])
